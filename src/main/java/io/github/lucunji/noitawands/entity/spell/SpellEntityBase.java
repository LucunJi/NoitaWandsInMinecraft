...
import io.github.lucunji.noitawands.spell.cast.TriggeredSpellPoolVisitor;
import io.github.lucunji.noitawands.util.MathHelper;
import io.github.lucunji.noitawands.util.NBTHelper;
...
public abstract class SpellEntityBase extends ThrowableEntity {
    ...
    protected void castSpellTrigger(RayTraceResult traceResult) {
        ...
        if (traceResult.getType() == RayTraceResult.Type.ENTITY) {
            ...
            Optional<Vector3d> hitPosOptional = box.rayTrace(this.getPositionVec(), this.getPositionVec().add(this.getMotion()));
    ...
    private void castSpell(Vector3d castVec) {
    ...
    protected void generateParticles() {
        ...
        Vector3d nextPos = this.getPositionVec().add(motionVec);
        if (this.isInWater()) {
            for (int j = 0; j < 4; ++j) {
